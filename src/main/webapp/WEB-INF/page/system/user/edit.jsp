			...
			<div class="panel-body">
				<form action="${pageContext.request.contextPath}${requestScope.record != null ?  '/system/user/update.do' :'/system/user/insert.do'  }" method="post" class="form form-horizontal" id="form">
					...
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<select class="input-text" id="ogid" name="ogid">
								<option value=0>--请选择--</option>
							<c:forEach items="${requestScope.organizations}" var="organization">
							   <option value="${organization.id}" <c:if test="${organization.id eq requestScope.record.ogid }">selected="selected"</c:if>>${organization.name}</option>
							</c:forEach>
							</select>
						</div>
					</div>
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<select class="input-text" id="rid" name="rid">
								<option value=0>--请选择--</option>
							<c:forEach items="${requestScope.roles}" var="role">
							   <option value="${role.id}" <c:if test="${role.id eq requestScope.record.rid }">selected="selected"</c:if>>${role.name}</option>
							</c:forEach>
							</select>
						</div>
					</div>
