			...
			<div class="panel-body">
				<form action="${pageContext.request.contextPath}${requestScope.record != null ?  '/device/devicereceive/update.do' :'/device/devicereceive/insert.do'}" method="post" class="form form-horizontal" id="form">
					...
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<input class ="input-text" type='<c:if test="${requestScope.record != null }">text</c:if><c:if test="${requestScope.record == null }">date</c:if>' id="receivedate" name="receivedate" value="<fmt:formatDate value="${record.receivedate }" pattern="yyyy-MM-dd"/>">
						</div>
					</div>
					<div class="row clearfix">
