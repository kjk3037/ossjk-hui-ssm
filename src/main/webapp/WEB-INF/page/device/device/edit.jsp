				...
				<form action="${pageContext.request.contextPath}${requestScope.record != null ?  '/device/device/update.do' :'/device/device/insert.do'}" method="post" class="form form-horizontal" id="form">
					...
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
						  <select id="ogid" name="ogid" class="input-text">
						     ...
						     <option value="${organization.id}"<c:if test="${organization.id eq record.ogid }">selected="selected"</c:if>>${organization.name}</option>
						     </c:forEach>
						  </select>
						</div>
					</div>
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<select id="dtid" name="dtid" class="input-text">
						     ...
						     <option value="${devicetype.id}" <c:if test="${devicetype.id eq record.dtid }">selected="selected"</c:if>>${devicetype.name}--${devicetype.brand}</option>
						     </c:forEach>
							</select>
						</div>
					</div>
					...
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							...
							<option value="1"<c:if test="${record.status eq 1}">selected="selected"</c:if>>入库</option>
					...
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<input type="text" class="input-text" value="<fmt:formatDate value="${requestScope.record.proddate }" pattern="yyyy-MM-dd"/>" placeholder="" id="proddate" name="proddate">
						</div>
					</div>
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
						     ...
						     <option value="${user.id}" <c:if test="${user.id eq record.creator }">selected="selected"</c:if>>${user.name}</option>
						     </c:forEach>
						  </select>
						</div>
					</div>
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<input type="text" class="input-text" value="<fmt:formatDate value="${requestScope.record.createtime }" pattern="yyyy-MM-dd"/>" placeholder="" id="createtime" name="createtime">
						</div>
					</div>
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
                           <select id="buyer" name="buyer" class="input-text">
						     <option value="0">--请选择--</option>
						     <c:forEach items="${requestScope.users}" var="user">
						     <option value="${user.id}"<c:if test="${user.id eq record.buyer }">selected="selected"</c:if>>${user.name}</option>
						     </c:forEach>
						  </select>
						</div>
					</div>
					<div class="row clearfix">
						...
						<div class="formControls col-xs-8 col-sm-9">
							<input type="text" class="input-text" value="<fmt:formatDate value="${requestScope.record.bugdate }" pattern="yyyy-MM-dd"/>" placeholder="" id="bugdate" name="bugdate">
						</div>
					</div>
					<div class="row clearfix">
